...
function FavoriteActivities({id}) {
  ...
  const handleAdd = (technologies, preferences) => {
    ...
    <section className="p-2 relative h-full flex justify-between">
      ...
      {
        show ? (
          <AddModal technologies={ profileTechnologies } favourites={ profilePreferences } show = {setShow} add={ handleAdd } />
        ) : null
      }

      {
        (userInformation?._id === id) ? (
          <button onClick={() => setShow(true)} className="bg-gray700 hover:scale-125 text-white rounded-full h-8 w-8 flex items-center justify-center absolute bottom-3 m-auto inset-x-0">
